...
func RunTransaction[T any](fn func(mongo.SessionContext) (T, error), opts ...RunTransactionOption) (T, error) {
	return RunTransactionWithContext(context.Background(), fn, opts...)
}

// run fn with mongodb transaction
func RunTransactionWithContext[T any](ctx context.Context,
	...
	return zero, fmt.Errorf("RunTransactionWithContext: transaction failed after %d attempts, last error: %w", maxRetry, err)
}
